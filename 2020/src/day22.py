...
def main():
    ...
    text = get_input()
    p1, p2 = map(lambda t: ints(t)[1:], text.split('\n\n'))
...
def score(p1: Sequence[int], p2: Sequence[int], recursive: bool) -> int:
    ...
    w, pw = (play_recursive if recursive else play)(p1, p2)
...
def play_recursive(p1: Deque[int], p2: Deque[int]) -> Tuple[bool, Deque[int]]:
    ...
    def freeze(p1_: Deque[int], p2_: Deque[int]):
        return tuple(p1_), tuple(p2_)

    while p1 and p2:
        h = freeze(p1, p2)
        if h in rounds:  # if the game state is exactly the same, end the game in P1's favor
            return True, p1
        rounds.add(h)
        ...
        if len(p1) >= p1c and len(p2) >= p2c:  # Recurse
            p1r = deque(list(p1)[:p1c])
            p2r = deque(list(p2)[:p2c])

...
main = do
    inp <- getContents
    let (seeds, maps) = parse inp
    putStrLn $ "Part 1: " ++ (part1 maps seeds)
    putStrLn $ "Part 2: " ++ (part2 maps seeds)


parse :: String -> ([Int], [[[Int]]])
parse inp = (seeds, maps)
    where (x:_:xs) = lines inp
          
          seeds = map read . drop 1 . words $ x
...
take2 _        = error "Not a multiple of 2"
